...
* 伪装界面布局改进：
  ...
  * 拖拽时显示跟随鼠标的模型名称标签，表示正在拖动的模型
  * 拖动内置模型时提示"内置模型无法移动"
* Bug 修复：
  * 修复了一些崩溃问题
  * 修复 Blockbench 导出的 Bedrock 动画中 step（步阶）关键帧被当作线性插值播放的问题：重写 `GeoAnimationParserMixin`，正确识别 `pre/post` 瞬变对格式的 step 帧并标记为 CONST 插值；修正了插值方向错误（BBS 关键帧段 a→b 的插值从后一帧 b 读取，旧实现错误标记了前一帧导致步帧完全不生效）；增加 try-catch 兜底，避免检测异常导致动画列表为空
