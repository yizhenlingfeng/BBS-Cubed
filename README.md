...
- 兼容说明：`MediaPlayer-BBS 1.0.1` 的 native 时间轴寻帧接口存在崩溃缺陷（部分机器上访问违规），BBS++ 会自动禁用其时间轴寻帧退回连续解码路径；`MediaPlayer-BBS 1.0.2+`（修复构建）会自动启用时间轴寻帧，拖动播放头逐帧精确定位。**修复构建发布时请将版本号改为 1.0.2 以上**
- 等待后续完善
