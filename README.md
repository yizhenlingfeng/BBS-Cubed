...
| Blockbench 程序路径    | 空   | 在伪装页右键用 Blockbench 打开用户模型前，在此选择 Blockbench.exe 的位置                                          |
| 优先使用 .bbmodel      | 开   | 模型目录下存在 .bbmodel 工程文件时，右键打开优先用它；否则打开 .geo.json                                          |
...
| **性能优化**     |      |                                                                                                                              |
| 模型方块渲染距离       | 0    | 原版固定 512 格。调低此值可减少远处模型方块的渲染与拾取开销。设为 0 则使用原版默认值                                        |
...
| 自定义快捷键可配置化         | Shift+W（选择当前时间点整列关键帧）等自定义快捷键已集成到 BBS UI 按键绑定设置的"关键帧编辑器"分类中，支持用户自由修改按键组合 |
...
* **修复隐藏伪装形态失效**：2.6 渲染链变更后，Mixin 目标从 BaseFilmController 迁移到 FilmEntityRenderer，恢复「隐藏伪装形态」功能
* **移除冲突功能**：结构伪装、视频伪装、逐骨骼 PBR、姿势材质栏、跟随轨道镜头模式等与 2.6 原生冲突的功能已移除
* **清理死代码**：删除 TextureThumbnailManager、UIGridFileLinkList、PBR UI 等废弃模块，移除 Gizmo 改版设置项
* **纹理补间增强**：像素溶解模式新增闪白与 PBR 发光支持，3D 扩散起点拾取器适配 2.6 拖拽系统

#### 3.3.3：
* 伪装页模型右键新增「用 Blockbench 编辑」：在 设置→增强界面 选择 Blockbench.exe 路径后，右键用户模型即可直接在 Blockbench 中打开；模型目录下存在 `.bbmodel` 工程文件时优先用它，否则打开 `.geo.json`（可在设置中开关）。路径未配置/无效或非 `.geo.json` 模型时按钮置灰，BBS 内置模型不显示该按钮
* 影片编辑器新增 `R` 键在当前与上一个镜头模式间循环切换；轨道控制器为常驻成员，切回后保留相机角度、距离与绑定实体
* 关键帧轨道汉化补全：补齐快捷栏物品槽、速度 vX/vY/vZ、VFX 增强、流体、PBR、摄像机 Clip、Mob、物品喷射等轨道的中英文名称
...
* 快捷键可配置化：Shift+W（选择当前时间点整列关键帧）已集成到UI按键绑定设置的"关键帧编辑器"分类中，支持自定义修改
